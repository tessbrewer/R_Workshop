...
Because we'll want to graph the gene expression data across timepoints, we want to make sure the timepoint values are considered numbers (continuous data) rather than characters (categorical data). We will therefore remove the "T" from every timepoint value and explicitly tell R to treat the resulting values as numbers rather than characters. This will take a minute to complete because the dataframe is very large.
